...
public class RelationExplorer {
	 ...
	 */
	static DirectedGraph<String, DefaultEdge> graph = new SimpleDirectedGraph<String, DefaultEdge>(
			DefaultEdge.class);
	...
	public void kill() {
		...
		try {
			while (!this.executor.awaitTermination(Long.MAX_VALUE,
					TimeUnit.DAYS)) {
			}
	...
	/**
	 * exploration method. Recursive in nature where each blast spawns threads
	 * and recurses
	 * 
	 * @param startNode
	 * @param endNode
	...
	private static List<QuerySolution> getAdjacentNodes(String startNode) {
		...
		try {
			String sparqlQueryString = "select distinct ?pred ?obj where {<"
					+ startNode
